...
在这个工作流中，你需要先在 [**输入动作编辑器**（Input Actions editor）](../../using-the-input-system/configuring-input.zh.md) 中配置动作，然后在代码中引用这些动作，并读取它们的值。
...
你可以通过动作和绑定功能，来将游戏或应用中概念意义上的动作与玩家真实用来执行的具体设备和控件分离开来，这可以使你的代码更加简洁且灵活。这种设计与 [在代码中直接读取设备状态](./direct.zh.md) 截然不同，直接读取可能适合简单的项目，但对于复杂的项目，那可能会变得难以管理且缺乏灵活性。

### 默认动作

输入系统预设了一些默认动作，例如“移动（Move）”“跳跃（Jump）” 等，这些动作适用于许多常见的应用和游戏场景。它们已经进行了配置并能读取常见的输入控制器，如键盘、鼠标、游戏手柄、触摸屏和 XR 控制器。

这意味着在大多数情况下，你可以直接开始编写脚本，并在脚本中引用已经为你配置好的默认动作名称，而无需进行任何额外配置。

## 从代码中访问动作

要在代码中访问动作，有多种方式可供选择，其中最简单的一种是使用 `FindAction` 方法。
...
!!! note "提示"
    ...
    查找并引用动作类似于查找并引用组件，如果你在 Unity 的其他地方对组件这样做过，那么你应该已经驾轻就熟了。

按照以下步骤使用 `FindAction` 获取动作引用并在脚本中读取用户输入：

1. 在 Unity 中创建一个新的 C# 脚本。

2. 在脚本顶部添加输入系统的“using”语句。这样才能在后文中使用输入系统的 API：

    ```C#
    using UnityEngine.InputSystem;
    ```

3. 在类主体中创建一些 `InputAction` 类型的变量，脚本中使用的每个动作对应一个变量。这些变量将引用并存储每个动作，规范的变量名称推荐以“Action”结尾。例如：
...
4. 在 `Start()` 方法中，使用 `FindAction` 查找并引用每个动作，并将其存储在相应的变量中，例如：
...
5. 在 `Update()` 方法中，从动作变量中读取值。这样代码就能每帧读取动作的值，并以此作出相应。

    读取值的方式取决于动作的**值类型**。具体而言，一些动作可能会返回 1D 或 2D 轴值，而另一些动作可能会返回 bool 的 true/false。在本例中，**Move** 动作返回一个 Vector2，而 **Jump** 动作返回一个 bool。
...
!!! note "提示"
    ...
    你应该避免在 `Update()` 循环中使用 `FindAction`，因为它是基于字符串的查找，而这可能会影响性能。正因如此你可以看到在上面的示例中，动作引用是在 `Start()` 函数期间查找的，并在找到后存储在变量中。

!!! note "提示"

    [InputSystem.actions :octicons-link-external-16:](https://docs.unity3d.com/Packages/com.unity.inputsystem@1.19/api/UnityEngine.InputSystem.InputSystem.html) API 只会指向被分配为 [项目级动作](../../using-the-input-system/project-wide-actions.zh.md) 的动作资产（Action Asset）。大多数项目只需要一个动作资产即可，但如果你使用了多个动作资产，则必须使用 `InputActionAsset` 类型的变量来引用那些你想访问的资产。
